...
class KGEModel(nn.Module):
    ...
    def forward(self, sample, ent_emb, rel_emb, time_emb, mode='single'):
        '''
        Forward function that calculate the score of a.md batch of triples.
        In the 'single' mode, sample is a.md batch of triple.
        ...
        if self.model_name in model_func:
            score = model_func[self.model_name](head, relation, tail, time, mode)
        else:
    ...
    def TComplEx(self, head, relation, tail, time, mode):
        ...
        if mode == 'head-batch':
            re_score = re_relation * re_tail + im_relation * im_tail
            im_score = re_relation * im_tail - im_relation * re_tail
            score = re_head * re_score * time + im_head * im_score * time
        else:
            re_score = re_head * re_relation - im_head * im_relation
            im_score = re_head * im_relation + im_head * re_relation
            score = re_score * re_tail * time + im_score * im_tail * time
    ...
    def TeRo(self, head, relation, tail, time, mode):
        ...
        re_time, im_time = torch.cos(re_time), torch.sin(im_time)
        real_h = re_head * re_time - im_head * im_time
        img_h = re_head * im_time - im_head * re_time

        real_t = re_tail * re_time - im_tail * im_time
        img_t = re_tail * im_time - im_tail * re_time

        if mode == 'head-batch':
            re_score = torch.abs(real_h + (re_relation - real_t))
            im_socre = torch.abs(img_h + (im_relation - img_t))
            score = re_score + im_socre
        else:
            re_score = torch.abs((real_h + re_relation) - real_t)
            im_socre = torch.abs((img_h + im_relation) - img_t)
            score = re_score + im_socre
